                ...
                <div class="table-responsive">
                    <table id="basic-datatables" class="display table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            $query = mysqli_query($koneksi, "SELECT * FROM users WHERE role = 'siswa' AND tipe_user = 'premium' ORDER BY id DESC");
                            $no = 1;
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?= !empty($data['masa_aktif']) ? date('d M Y', strtotime($data['masa_aktif'])) : 'N/A'; ?></td>
                                    <td>
                                        <span class="badge bg-warning">Premium</span>
                                    </td>
                                    <td>
                                        <div class="form-button-action">
                                            <a href=".?hal=langganan&search_user=<?= urlencode($data['email']); ?>" data-bs-toggle="tooltip" title="Edit Langganan"
                                                class="btn btn-link btn-primary btn-sm">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <a href="#" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal"
                                                data-id="<?= $data['id']; ?>" data-item-name="<?= htmlspecialchars($data['nama']); ?>" data-url-delete="?hal=proses_hapus_user" title="Hapus"
