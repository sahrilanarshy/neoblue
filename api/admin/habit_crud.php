...
switch($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            ...
            $q = $conn->query("SELECT * FROM habit ORDER BY id DESC");
            $data = [];
    ...
    case 'POST':
        ...
        $stmt = $conn->prepare("INSERT INTO habit (user_id,tanggal,tipe,status) VALUES (?,?,?,?)");
        $stmt->bind_param("isss", user_id, tanggal, tipe, status);
        if ($stmt->execute()) {
    ...
    case 'PUT':
        ...
        if (isset($put_vars['tanggal'])) { $fields[] = "tanggal=?"; $types.="s"; $values[] = $put_vars['tanggal']; }
        if (isset($put_vars['tipe'])) { $fields[] = "tipe=?"; $types.="s"; $values[] = $put_vars['tipe']; }
        ...
        $sql = "UPDATE habit SET ".implode(",", $fields)." WHERE id=?";
        $types .= "i";
        $values[] = $id;
