...
switch($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $id = (int)$_GET['id'];
            $stmt = $conn->prepare("SELECT * FROM event_kuis WHERE id=?");
            ...
            $q = $conn->query("SELECT * FROM event_kuis ORDER BY id DESC");
            $data = [];
    ...
    case 'POST':
        ...
        $stmt = $conn->prepare("INSERT INTO event_kuis (event_id,kuis_id) VALUES (?,?)");
        $stmt->bind_param("ii", event_id, kuis_id);
        if ($stmt->execute()) {
    ...
    case 'PUT':
        ...
        $types = "";
        $values = [];
        if (isset($put_vars['event_id'])) { $fields[] = "event_id=?"; $types.="i"; $values[] = (int)$put_vars['event_id']; }
        ...
        $sql = "UPDATE event_kuis SET ".implode(",", $fields)." WHERE id=?";
        $types .= "i";
        $values[] = $id;
    ...
    case 'DELETE':
        ...
        if (!$id) { echo json_encode(["success"=>false,"message"=>"Missing id"]); exit; }
        $stmt = $conn->prepare("DELETE FROM event_kuis WHERE id=?");
        $stmt->bind_param("i",$id);
