...
switch($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $id = (int)$_GET['id'];
            $stmt = $conn->prepare("SELECT * FROM user_badge WHERE id=?");
            ...
            $q = $conn->query("SELECT * FROM user_badge ORDER BY id DESC");
            $data = [];
    ...
    case 'POST':
        ...
        $stmt = $conn->prepare("INSERT INTO user_badge (user_id,badge_id) VALUES (?,?)");
    ...
    case 'PUT':
        ...
        if (isset($put_vars['badge_id'])) { $fields[] = "badge_id=?"; $types.="i"; $values[] = (int)$put_vars['badge_id']; }
        if (count($fields)==0) { echo json_encode(["success"=>false,"message"=>"Nothing to update"]); exit; }
        $sql = "UPDATE user_badge SET ".implode(",", $fields)." WHERE id=?";
        $types .= "i";
        $values[] = $id;
    ...
    case 'DELETE':
        ...
        if (!$id) { echo json_encode(["success"=>false,"message"=>"Missing id"]); exit; }
        $stmt = $conn->prepare("DELETE FROM user_badge WHERE id=?");
        $stmt->bind_param("i",$id);
