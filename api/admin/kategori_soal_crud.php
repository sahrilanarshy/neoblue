...
switch($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $id = (int)$_GET['id'];
            $stmt = $conn->prepare("SELECT * FROM kategori_soal WHERE id=?");
            ...
            $q = $conn->query("SELECT * FROM kategori_soal ORDER BY id DESC");
            $data = [];
    ...
    case 'POST':
        ...
        $stmt = $conn->prepare("INSERT INTO kategori_soal (nama_kategori,keterangan) VALUES (?,?)");
    ...
    case 'PUT':
        ...
        $types = "";
        $values = [];
        if (isset($put_vars['nama_kategori'])) { $fields[] = "nama_kategori=?"; $types.="s"; $values[] = $put_vars['nama_kategori']; }
        ...
        $sql = "UPDATE kategori_soal SET ".implode(",", $fields)." WHERE id=?";
        $types .= "i";
        $values[] = $id;
    ...
    case 'DELETE':
        ...
        if (!$id) { echo json_encode(["success"=>false,"message"=>"Missing id"]); exit; }
        $stmt = $conn->prepare("DELETE FROM kategori_soal WHERE id=?");
        $stmt->bind_param("i",$id);
